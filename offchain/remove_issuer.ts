...
async function main() {
    ...
    if (!issuerPkh) {
        throw new Error("Usage: ts-node remove_issuer.ts <issuer_pkh>");
    }
    ...
    const newDatumData: RegistryDatumType = {
        ...
        issuers: oldDatum.issuers.filter((pkh: string) => pkh !== issuerPkh),
    ...
    console.log("Issuer removed:", txHash);
}
